...
const resize = async (req, res, next) => {
  if (req.file) {
    try {
      ...
      await image.scaleToFit(100, 100).write(req.file.path);
      next();
    } catch (err) {
      next(err);
    }
...
const blur = async (req, _, next) => {
  if (req.file) {
    try {
      const image = await Jimp.read(req.file.path);
      await image.blur(9).write(req.file.path);
      next();
    } catch (err) {
      next(err);
    }
...
const writeOnImage = async (originalImagePath, outputMemePath, texts) => {
  try {
